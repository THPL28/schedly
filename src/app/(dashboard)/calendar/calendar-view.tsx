...
export default function CalendarView({ year, month, counts, appointmentsToday, selectedDate }: {
    ...
    return (
        <div className="flex flex-col lg:grid lg:grid-cols-[1fr_350px] gap-4 lg:gap-8 h-auto lg:h-[calc(100vh-150px)] relative">
            ...
            <div className="flex flex-col">
                <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4 lg:mb-6">
                    <div className="flex items-center gap-2 sm:gap-4">
                        <h1 className="text-xl sm:text-2xl font-bold text-foreground capitalize">{monthName} {year}</h1>
                        <button onClick={handleToday} className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-md border border-border bg-white cursor-pointer text-xs sm:text-sm font-semibold hover:bg-muted-light flex items-center">Hoje</button>
                    </div>

                    <div className="flex items-center gap-3 sm:gap-6 w-full sm:w-auto">
                        <button
                            onClick={() => setShowModal(true)}
                            className="btn btn-primary rounded-full px-4 sm:px-6 py-2 shadow-sm text-sm sm:text-base flex-1 sm:flex-initial"
                        >
                            <Plus size={18} className="sm:w-5 sm:h-5" /> <span className="hidden sm:inline">Criar</span>
                        </button>

                        <div className="flex items-center gap-2">
                            <button onClick={handlePrev} className="p-2 rounded-full hover:bg-muted-light cursor-pointer border-none bg-transparent flex items-center justify-center"><ChevronLeft size={18} className="sm:w-5 sm:h-5" /></button>
                            <button onClick={handleNext} className="p-2 rounded-full hover:bg-muted-light cursor-pointer border-none bg-transparent flex items-center justify-center"><ChevronRight size={18} className="sm:w-5 sm:h-5" /></button>
                        </div>
                    </div>
                </header>
            ...
            <div className="bg-white border border-border rounded-lg flex flex-col overflow-hidden h-auto lg:h-full order-first lg:order-last">
                <div className="p-4 sm:p-6 border-b border-border bg-muted-light/30">
                    <h2 className="text-base sm:text-lg font-semibold text-foreground capitalize m-0">
            ...
            {showModal && (
                <div className="fixed inset-0 w-full h-full bg-black/40 flex items-center justify-center z-50 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-lg w-full max-w-[400px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                        <div className="p-4 sm:p-6 border-b border-border flex justify-between items-center bg-gray-50">
                            <h3 className="m-0 text-base sm:text-lg font-semibold text-foreground">Novo Compromisso</h3>
