...
import { CalendarDays, ChevronRight, FileText, HeartPulse, Phone, Mail, Plus, ShieldCheck, UserRound, Pencil, CheckCircle2 } from 'lucide-react'
  ...
  return (
    <div className="page-shell">
      <div className="mb-5 flex items-center gap-2 text-xs text-slate-500">
        <Link href="/clients" className="hover:text-indigo-600">Clientes</Link><ChevronRight size={13} /><span className="text-slate-700">{patient.name}</span>
      </div>

      {query.saved === 'patient' && <div className="mb-5 flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800"><CheckCircle2 size={17} /> Dados clínicos salvos com sucesso.</div>}

      <header className="page-header">
        <div className="flex items-start gap-3"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-indigo-50 text-indigo-600"><UserRound size={20} /></div><div><h1 className="page-title">{isMedical ? 'Paciente' : 'Cliente'}: {patient.name}</h1><p className="page-description">{isMedical ? 'Dados do paciente e histórico de atendimentos.' : 'Dados e histórico de agendamentos.'}</p></div></div>
        <div className="page-actions">
          {isMedical && <Link href={`/clients/${id}/medical/edit`} className="ui-btn ui-btn-secondary"><Pencil size={16} /> Dados clínicos</Link>}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[320px_minmax(0,1fr)]">
        <aside className="space-y-4">
          <section className="section-card p-5"><div className="mb-4 flex items-center gap-2 text-sm font-bold text-slate-900"><UserRound size={16} className="text-indigo-600" /> Dados básicos</div><dl className="space-y-3 text-sm"><div><dt className="text-xs text-slate-400">E-mail</dt><dd className="mt-0.5 flex items-center gap-2 text-slate-700"><Mail size={13} />{patient.email}</dd></div><div><dt className="text-xs text-slate-400">Telefone</dt><dd className="mt-0.5 flex items-center gap-2 text-slate-700"><Phone size={13} />{patient.phone || '—'}</dd></div>{isMedical && <><div><dt className="text-xs text-slate-400">Nascimento</dt><dd className="mt-0.5 text-slate-700">{formatDate(patient.birthDate)}</dd></div><div><dt className="text-xs text-slate-400">Sexo</dt><dd className="mt-0.5 text-slate-700">{patient.sex || '—'}</dd></div></>}</dl></section>
          {isMedical && <section className="section-card p-5"><div className="mb-4 flex items-center gap-2 text-sm font-bold text-slate-900"><HeartPulse size={16} className="text-rose-600" /> Informações clínicas</div><div className="space-y-4 text-sm"><div><p className="text-xs text-slate-400">Alergias</p><p className="mt-1 whitespace-pre-wrap text-slate-700">{patient.allergies || 'Não informado'}</p></div><div><p className="text-xs text-slate-400">Medicamentos atuais</p><p className="mt-1 whitespace-pre-wrap text-slate-700">{patient.currentMedications || 'Não informado'}</p></div><div><p className="text-xs text-slate-400">Condições relevantes</p><p className="mt-1 whitespace-pre-wrap text-slate-700">{patient.relevantConditions || 'Não informado'}</p></div></div></section>}
