    ...
    for (const range of slots_base) {
        ...
        while (addMinutes(current, duration) <= end) {
            ...
            const isBlocked = appointments.some(appt => {
                ...
                // sum buffer solicited + buffer appointment
                const totalBuffer = buffer + apptBuffer;
                // expand appointment block by buffer on both sides
                const blockStart = addMinutes(apptStart, -totalBuffer);
                const blockEnd = addMinutes(apptEnd, totalBuffer);
                // return true if the slot overlaps with the blocked period
