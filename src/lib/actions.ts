...
const publicBookingSchema = z.object({
  ...
  rescheduleToken: z.string().uuid().optional(),
});
...
export async function bookAppointmentPublic(formData: FormData) {
    const parsed = publicBookingSchema.safeParse({
        ...
        rescheduleToken: formData.get('rescheduleToken') || undefined,
    })

    if (!parsed.success) {
        return { error: 'Dados invalidos. Revise os campos do agendamento.' }
    }
